...
### 2.2 Проверка оповещений
Для тестирования оповещений необходимо получить id чата, куда будут отправляться уведомления. Пользователь веб хука должен быть добавлен в целевой чат. Полное ID чата имеет вид chatXXXXXX, где Х - любая цифра от 0 до 9. В диалоге Тест необходимо заполнить поле To и проверить поле URL.

![Изображение](Image/Test_notify.png)

### 2.3 Возможные ошибки
- User not found - не найден чат с переданным id
- Вы не можете отправлять сообщения в указанный чат - не достаточно прав у пользователя вебхука
